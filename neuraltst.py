...
class NeuralNetwork():
    def __init__(self):
        ...
        self.synaptic_weights=np.array([[1.,1.,0.]]).T
    ...
    def train(self, training_set_inputs, training_set_outputs, number_of_training_iterations):
        for iteration in iter(range(number_of_training_iterations)):
            ...
            error=(iteration, str(numpy.mean(numpy.abs(error))))
            # Adjust the weights.
            self.synaptic_weights += adjustment
            if (iteration % 1 == 0):
                print ("error after %s iterations: %s"),
                print error
                if (float(error[1])<0.05):
                    print("iterations="),
                    print(iteration)
                    break
...
if __name__ == "__main__":
    ...
    # Do it 10,000 times and make small each adjustments time.
